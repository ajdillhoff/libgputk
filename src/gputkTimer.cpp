...
static inline gpuTKTimerNode_t
gpuTKTimerNode_new(int idx, gpuTKTimerKind_t kind, const char *file,
...
gpuTKTimerNode_toJSONObject(gpuTKTimerNode_t node) {
    if (node == nullptr) {
...
static inline void gpuTKTimerNode_toLogger(gpuTKTimerNode_t node) {
    ...
    int parent_id = gpuTKTimerNode_hasParent(node)
                        ...
                        : -1;
    gpuTKLog(
        INFO, "[TIME][", _nodeKind(gpuTKTimerNode_getKind(node)), "][",
        gpuTKTimerNode_getMessage(node), "][", gpuTKTimerNode_getEndFile(node),
        ": ", gpuTKTimerNode_getStartLine(node), "-",
        gpuTKTimerNode_getEndLine(node), "] Elapsed time: ",
        static_cast<double>(gpuTKTimerNode_getElapsedTime(node)) * 1e-6, " ms");
    return;
}
...
static inline string gpuTKTimerNode_toJSON(gpuTKTimerNode_t node) {
    if (node == nullptr) {
        return "";
    } else if (GPUTK_USE_JSON) {
        nlohmann::json json = gpuTKTimerNode_toJSONObject(node);
        return json.dump();
    } else {
        stringstream ss;
        ...
        ss << gpuTKString_quote("mpi_rank") << ":"
           << gpuTKTimerNode_getMPIRank(node) << ",\n";
        ...
        ss << gpuTKString_quote("end_time") << ":"
           << gpuTKTimerNode_getEndTime(node) << ",\n";
        ss << gpuTKString_quote("elapsed_time") << ":"
           << gpuTKTimerNode_getElapsedTime(node) << ",\n";
        ss << gpuTKString_quote("start_line") << ":"
           << gpuTKTimerNode_getStartLine(node) << ",\n";
        ss << gpuTKString_quote("end_line") << ":"
           << gpuTKTimerNode_getEndLine(node) << ",\n";
...
static inline string gpuTKTimerNode_toXML(gpuTKTimerNode_t node) {
    if (node == nullptr) {
        ...
        ss << "<start_file>" << gpuTKTimerNode_getStartFile(node)
           << "</start_file>\n";
        ss << "<end_file>" << gpuTKTimerNode_getEndFile(node)
           << "</end_file>\n";
        ss << "<parent_id>"
           << gpuTKString(
                  gpuTKTimerNode_hasParent(node)
...
static nlohmann::json gpuTKTimer_toJSONObject(gpuTKTimer_t timer) {
...
string gpuTKTimer_toJSON(gpuTKTimer_t timer) {
    if (timer == nullptr) {
        ...
        nlohmann::json json = gpuTKTimer_toJSONObject(timer);
        return json.dump();
    } else {
        stringstream ss;
        gpuTKTimerNode_t iter;
...
string gpuTKTimer_toJSON() { return gpuTKTimer_toJSON(_timer); }
...
string gpuTKTimer_toXML() { return gpuTKTimer_toXML(_timer); }
...
static inline gpuTKTimerNode_t _findNode(gpuTKTimer_t timer,
                                         gpuTKTimerKind_t kind, string msg) {
    gpuTKTimerNode_t iter;
...
void gpuTKTimer_stop(gpuTKTimerKind_t kind, string msg, const char *file,
    ...
    gpuTKTimerNode_setElapsedTime(node, currentTime -
                                            gpuTKTimerNode_getStartTime(node));
