...
static inline nlohmann::json gpuTKLogEntry_toJSONObject(gpuTKLogEntry_t elem) {
    if (elem == nullptr) {
...
static inline string gpuTKLogEntry_toJSON(gpuTKLogEntry_t elem) {
    if (elem == NULL) {
        ...
        nlohmann::json json = gpuTKLogEntry_toJSONObject(elem);
        return json.dump();
    } else {
        stringstream ss;
...
static inline string gpuTKLogEntry_toXML(gpuTKLogEntry_t elem) {
    if (elem != nullptr) {
        ...
        ss << "<function>" << gpuTKLogEntry_getFunction(elem)
           << "</function>\n";
...
string gpuTKLogger_toJSON() { return gpuTKLogger_toJSON(_logger); }
static nlohmann::json gpuTKLogger_toJSONObject(gpuTKLogger_t logger) {
...
string gpuTKLogger_toXML() { return gpuTKLogger_toXML(_logger); }
